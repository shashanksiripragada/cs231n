...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dA1,grads['W2'],grads['b2'] = affine_backward(dscores,cache3)
        
        dZ1 = relu_backward(dA1,cache2)
        
        dX,grads['W1'],grads['b1'] = affine_backward(dZ1,cache1)
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):            
             ...
             if (self.normalization in ['batchnorm','layernorm']) and (i!=self.num_layers-1):
                 self.params['gamma%d' %(i+1)] = np.ones((total_dims[i+1]))
                 self.params['beta%d' %(i+1)] = np.zeros((total_dims[i+1]))
    ...
    def loss(self, X, y=None):
        ...
        for i in reversed(range(1,self.num_layers+1)):
            ...
            if i==self.num_layers:
               ...
               if self.use_dropout:
                    dscores = dropout_backward(dscores, caches['dropout%d' %(i)])
