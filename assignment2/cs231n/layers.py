...
def batchnorm_backward_alt(dout, cache):
    ...
    # refer to https://kevinzakka.github.io/2016/09/14/batch_normalization/
    '''
    delta_x = inv_sqrt_var*((1/N)*(inv_sqrt_var**2)*((x-sample_mean)*(x-sample_mean)) - ((1-(1/N))*np.ones((N,D))))
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    out_len = 1 + (H + 2 * pad - HH) // stride #get int
    ...
    for i in range(N): #no of examples
        for j in range(F): #no of filters
            for k in range(out_len): #output height
                for l in range(out_wid): #output width
                    out[i,j,k,l] = np.sum(pad_x[i,:,k*stride:k*stride+HH,l*stride:l*stride+WW]*w[j]) + b[j]
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(N):
        for j in range(C):
            for k in range(out_h):
                for l in range(out_w):
                    # find indices of max_pool
                    ind = np.unravel_index(np.argmax(x[i,j,k*stride:k*stride+pool_height,l*stride:l*stride+pool_width]),
                                    (pool_height,pool_width))
                    
                    dx[i,j,k*stride:k*stride+pool_height,l*stride:l*stride+pool_width][ind] = dout[i,j,k,l]
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    x_n = np.reshape(np.transpose(x,(0,2,3,1)),(-1,C))
    
    out,cache = batchnorm_forward(x_n, gamma, beta, bn_param)
...
def spatial_batchnorm_backward(dout, cache):
    ...
    dout_n = np.reshape(np.transpose(dout,(0,2,3,1)),(-1,C))
    
    dx, dgamma, dbeta = batchnorm_backward_alt(dout_n, cache)
